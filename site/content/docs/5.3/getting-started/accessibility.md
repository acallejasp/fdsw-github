...
On browsers that support `prefers-reduced-motion`, and where the user has *not* experiencialicitly signaled that they'd prefer reduced motion (i.e. where `prefers-reduced-motion: no-preference`), Bootstrap enables smooth scrolling using the `scroll-behavior` property.
